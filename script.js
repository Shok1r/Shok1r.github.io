function bdCanvas() {
    ...
    opts = {
        minRadius: 0.5,
        maxRadius: 1.4,
        colors: ["rgba(255, 255, 255, 0.7)", "rgba(252, 244, 201, 0.7)",
                 "rgba(201, 252, 201, 0.7)", "rgba(201, 236, 252, 0.7)",
                 "rgba(229, 201, 252, 0.7)", "rgba(252, 201, 201, 0.7)",
    ...
    Stars = function (w, h, input = false) {
        if (input) {
            this.x = w;
            this.y = h;
        } else {
            this.x = Math.random() * w;
            this.y = Math.random() * h - (h * 0.18);
        }
    ...
    canvas.addEventListener('click', function(event) {
        arrStars.push(new Stars(event.clientX, event.clientY, true));
    });
